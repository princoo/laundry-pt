...
export async function getRequestForStaffEdit(id: string, actor: Actor) {
  const found = await prisma.request.findUnique({
    where: { id },
    select: {
      id: true, roomNumber: true, guestName: true, note: true,
      isHanger: true, isExpress: true, status: true,
      needsChanges: true, flagReason: true, assignedToId: true,
      ...
      items: {
        select: { laundryItemId: true, serviceType: true, quantity: true, unitPrice: true },
      },
    },
  })
  if (!found) return null
  if (!canManageRequest(found.assignedToId, actor)) throw new ForbiddenRequestAccessError()
  return found
}

export async function editStaffRequest(id: string, input: StaffEditInput, actor: Actor) {
  const existing = await prisma.request.findUnique({
    ...
    select: {
      ...
      needsChanges: true,
      assignedToId: true,
      items: { select: { laundryItemId: true, serviceType: true, unitPrice: true } },
    },
  })
  if (!existing) throw new StaffRequestNotFoundError('Request not found.')
  if (!canManageRequest(existing.assignedToId, actor)) throw new ForbiddenRequestAccessError()

  if (!FLAG_ELIGIBLE_STATUSES.includes(existing.status)) {
    throw new StaffRequestNotEditableError(
      FLAG_LOCKED_REASONS[existing.status] ?? 'This request can no longer be corrected.'
    )
  }
  ...
  if (!existing.needsChanges) throw new StaffRequestNotEditableError(NOT_FLAGGED)

  // Lines already on the request keep the price the guest agreed to; only new
  // ones are priced from today's catalogue.
  const orderItems = await priceRequestItems(input.items, existingPriceMap(existing.items))
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (count === 0) throw new StaffRequestNotEditableError(FLAG_CLEARED_MID_EDIT)
