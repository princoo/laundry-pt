    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          {kindLabel && (
            <div
              className={`text-[11px] font-medium mb-0.5 ${NOTIFICATION_KIND_CLASSES[kind]}`}
            >
              {kindLabel}
            </div>
          )}
          <div className="text-sm font-medium text-salt-text truncate">
            Room {roomNumber}
            {guestName && (
              <span className="font-normal text-salt-text-sec">
                - {guestName}
              </span>
            )}
          </div>
        ...
        <div className="text-right shrink-0">
          <div className="text-sm text-salt-text">
