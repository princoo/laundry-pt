...
export function ReportsRevenueChart({ revenueByDay, from, to }: Props) {
  ...
  if (revenueByDay.length <= 2) {
    return (
      <div className="bg-white rounded-xl border border-[0.5px] border-salt-border shadow-sm p-5 sm:p-6">
        <h2 className="text-sm font-medium text-salt-text mb-4">Revenue by day</h2>
  ...
  return (
    <div className="bg-white rounded-xl border border-[0.5px] border-salt-border shadow-sm p-5 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-salt-text-muted">Peak {formatCurrency(max)}</span>
      </div>
      <div className="flex items-end gap-1 h-40 border-b border-[0.5px] border-salt-border">
        {bars.map((bar) => (
          <div
            key={bar.label}
            title={bar.tooltip}
            className="flex-1 bg-salt-navy hover:bg-salt-navy-hover transition-colors rounded-t min-w-[2px]"
