...
export function BulkRoomQrGrid() {
  ...
  const activeRanges = ROOM_RANGES.map((range, i) => ({ range, i })).filter(({ i }) =>
    selected.has(i),
  )
  const totalRooms = activeRanges.reduce((sum, { range }) => sum + roomsInRange(range).length, 0)
    ...
    <div>
      ...
      <div className="print:hidden mb-6 bg-white border border-[0.5px] border-salt-border rounded-xl shadow-sm p-5 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-[11px] uppercase tracking-wide text-salt-text-muted">Room ranges</p>
      ...
      <div className="hidden print:block mb-4">
        <h2 className="text-base font-bold text-salt-text">SALT of Akagera — Room QR codes</h2>
      </div>
            ...
            <section key={i} className="break-inside-avoid">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 print:grid-cols-3 gap-3 sm:gap-4">
                {roomsInRange(range).map((room) => (
                  <BulkRoomQrCard key={room} room={room} url={buildGuestRoomUrl(room)!} />
                ))}
              </div>
            </section>
          ))}
