      ...
      <div className="flex flex-col gap-4">
        {changes.map((change, index) => (
          ...
          >
            ...
            {change.reason && (
              <p className="text-xs text-salt-text-sec italic">
                {change.reason}
              </p>
            )}
            ...
            {change.lines.map((line, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex gap-1.5 text-xs text-salt-text-sec"
                >
                  <Icon
                    className={`w-3 h-3 shrink-0 mt-0.5 ${KIND_CLASS[line.kind]}`}
                  />
                  <span>
            ...
            {change.totalFrom !== change.totalTo && (
              <div className="text-xs text-salt-text pl-[18px]">
                Total {formatCurrency(change.totalFrom)} →{" "}
                {formatCurrency(change.totalTo)}
              </div>
            )}
          </div>
        ))}
