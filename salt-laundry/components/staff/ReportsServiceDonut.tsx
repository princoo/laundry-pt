...
export function ReportsServiceDonut({ byServiceType, serviceRevenue }: Props) {
  const percents = SEGMENTS.map((seg) =>
    serviceRevenue > 0
      ? (byServiceType[seg.type].revenue / serviceRevenue) * 100
      : 0,
  );
  ...
  return (
    <div className="bg-white rounded-xl border border-[0.5px] border-salt-border shadow-sm p-5 sm:p-6 flex flex-col items-center">
      <h2 className="text-sm font-medium text-salt-text mb-4 self-start">
        Share of revenue
      </h2>
        ...
        {SEGMENTS.map((seg) => (
          ...
          >
            <span className="flex items-center gap-2 text-salt-text-sec">
              <span
                className={`w-2.5 h-2.5 rounded-full bg-current ${seg.colorClass}`}
              />
              {SERVICE_TYPE_LABELS[seg.type]}
            </span>
            <span className="text-salt-text">
              {formatCurrency(byServiceType[seg.type].revenue)}
            </span>
          </div>
        ))}
      </div>
