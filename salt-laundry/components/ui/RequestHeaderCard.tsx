  ...
  return (
    <div className="bg-white rounded-xl border border-[0.5px] border-salt-border shadow-sm p-4 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
        <div>
          <div className="text-2xl sm:text-[26px] font-medium text-salt-text leading-tight">
            Room {roomNumber}
          </div>
          {guestName && (
            <div className="text-sm text-salt-text-sec mt-1">{guestName}</div>
          )}
          {reference && (
            <div className="text-xs text-salt-text-muted mt-1 font-mono">
              {reference}
            </div>
          )}
        </div>
      ...
      {note && (
        <div className="bg-salt-cream rounded-lg px-4 py-3 mt-4">
          <div className="text-[11px] uppercase text-salt-text-muted">
            Guest note
          </div>
