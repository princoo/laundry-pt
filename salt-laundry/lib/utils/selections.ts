...
export function setQuantity(
  ...
  if (quantity <= 0) {
    return Object.fromEntries(Object.entries(selections).filter(([id]) => id !== item.id))
  }

  const serviceType = selections[item.id]?.serviceType ?? initialServiceFor(item, defaultServiceType)
...
export function applyDefaultService(
  ...
  return Object.fromEntries(
    Object.entries(selections).map(([id, selection]) => {
      ...
      return [id, canApply ? { ...selection, serviceType: defaultServiceType } : selection]
    })
  )
}
