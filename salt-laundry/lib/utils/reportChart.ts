...
export function buildRevenueChartBars(
  ...
  if (totalDays <= 31) {
    return revenueByDay.map(({ date, total }) => {
      ...
      return {
        label,
        total,
        tooltip: `${label}- RWF ${total.toLocaleString()}`,
      };
    });
  }
  ...
  for (const { date, total } of revenueByDay) {
    const dayOffset = Math.floor(
      (asCalendarDate(date).getTime() - fromDate.getTime()) / MS_PER_DAY,
    );
  ...
  return Array.from(buckets.entries())
    .sort(([a], [b]) => a - b)
    .map(([week, total]) => {
      const weekStart = toCalendarKey(
        new Date(fromDate.getTime() + week * 7 * MS_PER_DAY),
      );
      const label = `Wk ${formatDayLabel(weekStart)}`;
      return {
        label,
        total,
        tooltip: `Week of ${formatDayLabel(weekStart)}- RWF ${total.toLocaleString()}`,
      };
    });
}
