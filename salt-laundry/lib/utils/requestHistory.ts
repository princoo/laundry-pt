...
const byItem = (lines: HistoryLine[]) =>
  new Map(lines.map((line) => [line.laundryItemId, line]));
...
function describeLine(before: HistoryLine, after: HistoryLine): string[] {
  ...
  if (before.unitPrice !== after.unitPrice) {
    details.push(
      `Unit price ${formatCurrency(before.unitPrice)} → ${formatCurrency(after.unitPrice)}`,
    );
  }
  return details;
}
  ...
  for (const line of before) {
    ...
    if (!next) {
      changes.push({
        ...
        nameEn: line.nameEn,
        details: [
          `${line.quantity} × ${SERVICE_TYPE_LABELS[line.serviceType]}`,
        ],
      });
      continue;
    }
    ...
    if (details.length > 0)
      changes.push({ kind: "edited", nameEn: line.nameEn, details });
  }
...
export function diffFields(
  ...
  if (before.isExpress !== after.isExpress) {
    fields.push({
      label: "Express",
      from: yesNo(before.isExpress),
      to: yesNo(after.isExpress),
    });
  }
  if (before.isHanger !== after.isHanger) {
    fields.push({
      label: "Handling",
      from: before.isHanger ? "On hanger" : "Folded",
