...
// return deadline.
export function hotelDateAtHour(instant: Date, hour: number, dayOffset = 0): Date {
  const p = hotelParts(instant)
...
export function parseHotelDayStart(date: string): Date | null {
  ...
  return parsed && hotelWallTime(parsed.year, parsed.month, parsed.day, 0, 0, 0)
}
...
export function parseHotelDayEnd(date: string): Date | null {
  ...
  return new Date(hotelWallTime(parsed.year, parsed.month, parsed.day + 1).getTime() - 1)
}
