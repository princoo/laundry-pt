...
export async function authorizeSoa(
  ...
  if (profile.status === "INACTIVE") {
    console.warn(
      `[soa] refused ${profile.id}: SOA reports the account inactive`,
    );
    return null;
  }
  ...
  if (expiry === null) {
    console.warn(
      `[soa] refused ${profile.id}: missing or already-past expiresAt`,
    );
    return null;
  }
  ...
  try {
    ...
    const why =
      error instanceof EmailBelongsToAnotherUserError
