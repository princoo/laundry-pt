...
import { soaProfileSchema, type SoaProfile } from '@/lib/validations/soaProfile.schema'
...
export async function fetchSoaProfile(token: string): Promise<SoaProfile | null> {
  if (!SOA_API_URL) {
    console.error('[soa] SOA_API_URL is not set — cannot verify a sign-in token')
    return null
  }
  ...
  try {
    ...
    console.error(`[soa] ${SOA_ME_PATH} did not answer:`, (error as Error)?.name ?? error)
    return null
  }
}
