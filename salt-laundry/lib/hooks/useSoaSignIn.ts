...
import {
  EXPIRES_AT_PARAM, REDIRECT_PARAM, SOA_PROVIDER_ID, TOKEN_PARAM,
} from '@/lib/constants/soa'

const NO_TOKEN = 'This link arrived without a sign-in token. Start again from SOA.'
const REFUSED = 'That sign-in could not be completed. It may have expired — start again.'
...
export function useSoaSignIn() {
  ...
  const exchange = useCallback(async () => {
    if (!token) return
    try {
      const result = await signIn(SOA_PROVIDER_ID, { token, expiresAt, redirect: false })
      if (result?.error) return setHasFailed(true)
