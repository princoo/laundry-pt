...
export const MY_TASK_STATUS_FILTERS = ["ALL", ...REQUEST_STATUSES] as const;
...
export const FLAG_LOCKED_REASONS: Partial<Record<RequestStatus, string>> = {
  DELIVERED:
    "This request has been delivered and billed, so it can no longer be flagged for changes.",
...
export const EDIT_LOCKED_REASONS: Record<RequestStatus, string> = {
  PENDING: "",
  COLLECTED:
    "Your items have been collected, so this request can no longer be edited.",
  IN_PROGRESS:
    "Your items are being cleaned, so this request can no longer be edited.",
